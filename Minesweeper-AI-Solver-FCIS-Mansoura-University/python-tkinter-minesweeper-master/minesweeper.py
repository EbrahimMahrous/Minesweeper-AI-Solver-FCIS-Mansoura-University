#هذا السطر يستورد جميع المكونات (widgets) والدوال اللازمة من مكتبة Tkinter، وهي مكتبة تستخدم لبناء واجهات المستخدم الرسومية في Python.
from tkinter import *
...
class Minesweeper:
    ...
    def __init__(self, tk):
        ...
        # Add Solve buttons
        self.solve_dfs_button = Button(self.frame, text="Solve (DFS)", command=self.solve_dfs)
        self.solve_dfs_button.grid(row=SIZE_X+2, column=0, columnspan=int(SIZE_Y/3), sticky="WE")

        self.solve_bfs_button = Button(self.frame, text="Solve (BFS)", command=self.solve_bfs)
        self.solve_bfs_button.grid(row=SIZE_X+2, column=int(SIZE_Y/3), columnspan=int(SIZE_Y/3), sticky="WE")

        self.solve_greedy_button = Button(self.frame, text="Solve (Greedy)", command=self.solve_greedy)
        self.solve_greedy_button.grid(row=SIZE_X+2, column=2*int(SIZE_Y/3), columnspan=int(SIZE_Y/3), sticky="WE")

        self.solve_csp_button = Button(self.frame, text="Solve (CSP)", command=self.solve_csp)
        self.solve_csp_button.grid(row=SIZE_X+3, column=0, columnspan=int(SIZE_Y/3), sticky="WE")
    ...
    def onClick(self, tile):
        ...
        if tile["mines"] == 0:
            ...
            self.dfs(tile["coords"]["x"], tile["coords"]["y"])  # Perform DFS here
            # self.bfs(tile["coords"]["x"], tile["coords"]["y"])  # Perform BFS here
        else:
    ...
    def dfs(self, x, y):
        tile = self.tiles[x][y]
        if tile["state"] != STATE_DEFAULT or tile["isMine"]:
            return
        ...
        if tile["mines"] == 0:
            for neighbor in self.getNeighbors(x, y):
                self.dfs(neighbor["coords"]["x"], neighbor["coords"]["y"])
    ...
    def bfs(self, x, y):
        ...
        while queue:
            ...
            if tile["state"] != STATE_DEFAULT or tile["isMine"] or (x, y) in visited:
                continue
            ...
            if tile["mines"] == 0:
                for neighbor in self.getNeighbors(x, y):
                    queue.append((neighbor["coords"]["x"], neighbor["coords"]["y"]))
    ...
    def greedy(self, x, y):
        ...
        while stack:
            ...
            if tile["mines"] == 0:
                neighbors = sorted(self.getNeighbors(x, y), key=lambda tile: tile["mines"])
                stack.extend([(neighbor["coords"]["x"], neighbor["coords"]["y"]) for neighbor in neighbors])
    ...
    def csp(self):
        ...
        if not unopened_cells:
            return  # لا توجد خلايا غير مفتوحة

        # اختر أول خلية غير مفتوحة لتبدأ
        x, y = unopened_cells[0]

        # قم بفتح الخلية الأولى وجعلها مرئية
    ...
    def reveal_adjacent(self, x, y):
        ...
        while stack:
            ...
            if tile["mines"] == 0:
                ...
                for neighbor in self.getNeighbors(x, y):
                    stack.append((neighbor["coords"]["x"], neighbor["coords"]["y"]))
            else:
                tile["button"].config(image=self.images["numbers"][tile["mines"]-1])
                tile["state"] = STATE_CLICKED
                self.clickedCount += 1
    ...
    def solve_dfs(self):
        self.solve_with_algorithm(self.dfs)

    def solve_bfs(self):
        self.solve_with_algorithm(self.bfs)

    def solve_greedy(self):
        self.solve_with_algorithm(self.greedy)
